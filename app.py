...
import nltk
nltk.download('stopwords', download_dir='C:\\Users\\gagan\\PycharmProjects\\pythonProject2\\venv\\nltk_data')
...
def fetch_yt_video(link):
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info_dict = ydl.extract_info(link, download=False)  # Set download=False to only get info
...
connection = pymysql.connect(host='localhost', user='root', password='123qwerty', db='cv')
...
def run():
    ...
    link = '[©Developed by Gagan](https://github.com/Gagandeep010/)'
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                if 'Achievements' or 'Certification' in resume_text:
                    resume_score = resume_score + 20
                    st.markdown(
                ...
                if 'Projects' or 'Publications' in resume_text:
                    resume_score = resume_score + 20
                    st.markdown(
        ...
        if st.button('Login'):
            if ad_user == 'Gagan' and ad_password == 'gagan123':
                st.success("Welcome Gagan !")
                # Display Data
                ...
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Resume Score', 'Timestamp', 'Total Page',
                                                 ...
                                                 'Recommended Course'])
                st.dataframe(df)
                df['Status'] = df['Resume Score'].astype(int).apply(
                    lambda score: 'Accepted' if score > 60 else 'Rejected')


                # Display separate sections for accepted and rejected candidates
                st.subheader("**Accepted Users**")
                st.dataframe(df[df['Status'] == 'Accepted'])

                st.subheader("**Rejected Users**")
                st.dataframe(df[df['Status'] == 'Rejected'])

                # Optional: Add a download link for the complete data
                download_link = get_table_download_link(df, 'user_data.csv', 'Download User Data CSV')
                ...
                # Group data by 'Predicted Field' and get counts
                df = df.applymap(lambda x: x.decode() if isinstance(x, bytes) else x)
                df = df.applymap(lambda x: x.decode() if isinstance(x, bytes) else x)

                # Check if 'Predicted Field' column exists
                if 'Predicted Field' not in df.columns:
                    print("Error: 'Predicted Field' column not found in the DataFrame.")
                    return

                # Print column names and the first few rows for debugging purposes
                ...
                # Check the content of field_counts
                print("Field Counts DataFrame:", field_counts)

                # Create the pie chart with the grouped data
                ...
                for column in plot_data.columns:
                    if plot_data[column].dtype == 'object':  # or 'bytes'
                        plot_data[column] = plot_data[column].astype(str)

                user_level_counts = plot_data['User_level'].value_counts().reset_index()
                user_level_counts.columns = ['User Level', 'Count']  # Rename for clarity

                # Extract labels and values from the summary DataFrame
                labels = user_level_counts['User Level']
                values = user_level_counts['Count']
                st.subheader("**Pie-Chart for User's Experienced Level**")
                # fig = px.pie(df, values=values, names=labels, title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
                # st.write("Data columns for Experienced Level:", df.columns)

                # Assuming your DataFrame has columns 'Experienced Level' and 'Experience Count'
                fig = px.pie(user_level_counts, values='Count', names='User Level',title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
                st.plotly_chart(fig)
